...
def extract_structured_record(line: str) -> SectionRow:
    ...
    """
    prompt = "return the following line in the form of exclusively a working JSON set up with the different categories: program (should be 3 capital letters), number(should be 3 numbers with the possibilty of an L at the end), section(just a lowercase letter and if not present just assign a lowercase a. IT WILL ALWAYS BE LOWERCASE), title(look for a string in title case), credits(look for a float), days(a list that may look like -m-w-f-. if not present or is ------- represent as null), times (if not given list as null), room(4 letters followed by 3 numbers after a space these numbers will always be present if the four letters are present if ot present list as null), faculty(someones first inital and last name there may also be an instance hwere two faculty are present then it would look like Stephenson/Tomb), tags(Possible tags are: A, S, G, R, E1, E2, E3, D. Tags may or may not be present in the line. If no tags from this list appear, set tags equal to null. Tags are typically at the end of the line there are no individual tags that are more then 2 characters without a comma being invloved. Hey guy here you fucking dipshit stop putting none when there are in fact tags you blind whore) here is an example line: THR 117 Acting-I 3 a K. Anderson 12:40-2:10PM --T-R-- GRNT 502 E1, A, THR would be the program, 117 would be the number, the section would be a, the faculty would be K. Anderson, the times would be 12:40-2:10PM, the days would be --T-R--,  the room would be GRNT 502 and the tag for instance would be A in this situation be aware these will not always be in order in the following line :" + line 
